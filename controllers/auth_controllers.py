...
def auth_token_add():
    ...
    if user_query:
        ...
        existing_tokens = db.session.query(AuthTokens).filter(AuthTokens.email == user_id).all()

        if existing_tokens:
            for token in existing_tokens:
                if token.expiration_date < expiration_datetime:
        ...
        return jsonify({"message":"auth successs","auth_info":auth_token_schema.dump(new_token)}),201
